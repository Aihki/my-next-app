...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!formData.get('title') || !formData.get('description') || !formData.get('genre') || !formData.get('series')) {
        return new NextResponse('Title, description, book genre and series name are required', { status: 400 });
        }

    const mediaItem: Omit<MediaItem, 'book_id' | 'created_at' |'thumbnail'> = {
        title: formData.get('title') as string,
        description: formData.get('description') as string,
        book_genre: formData.get('genre') as string,
        series_name: formData.get('series') as string,
